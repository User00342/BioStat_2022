...
ggplot(data, aes(x =Repeat.Instrument)) +
  geom_bar(fill="#66CCFF", colour="black") +
  theme_minimal() +
...
dataHIV <-data
dataHIV[dataHIV == ''] <- NA 
dataHIV <- dataHIV %>% filter(is.na(ВИЧ.инфекция) == FALSE)
dataHIV$ВИЧ.инфекция <- ifelse(dataHIV$ВИЧ.инфекция == "отрицает", '0', '1') %>% as.factor()

```
```{r}
dataDH <- dataHIV %>% dplyr::select(where(is.factor)) %>% dplyr::select(!contains(c('Record.ID', 'Дата', "Причины", 'Время', 'Лечение',  "Complete", "What.", "Do.you.eat.any.of.the.", "Where.", "How.do.you", "Do.you.", "Services", "С.какого", "In.the.past.year.","Communicable.Languages.", "Check.if.this.person.", "Have.you.")))%>%
  dplyr::select_if(~!(all(is.na(.)) | all(. == ""))) %>% filter(ВИЧ.инфекция == "1")



#name <- colnames(dataDH)[1]
#tab2 <- fct_count(dataDH[,1]) %>% add_column(`Переменная` = name)
#for (i in 2:length(colnames(dataDH))) {
#  name <- colnames(dataDH)[i]
#  tab1 <- fct_count(dataDH[,i]) %>% add_column(`Переменная` = name)
#  tab2 <- tab1 %>% bind_rows(tab2)
#}
...
logit %>% broom::tidy(exponentiate = TRUE, conf.int = TRUE) %>% data.frame() %>% flextable()  %>% theme_box()
...
l <- logit %>% broom::tidy(exponentiate = TRUE, conf.int = TRUE)%>% flextable() %>% theme_box()
...
tabl %>% dplyr::select(!`Period`) %>% table() %>% as.data.frame.matrix() %>% rownames_to_column("Значения") %>% flextable()  %>% theme_box() 
cbbPalette <- c("#000000", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7") 
